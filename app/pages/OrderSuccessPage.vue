            ...
            <div v-else-if="orderData" class="max-w-3xl mx-auto bg-white rounded-lg shadow-sm p-8 text-center">
                ...
                <div class="bg-blue-50 rounded-lg p-6 mb-8 text-left">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <p class="font-medium">
                                {{ orderData.payment === "invoice" ? "Безналичный расчет" : orderData.payment === "card" ? "Оплата картой" : "Не указан" }}
                            </p>
                        </div>
                        <div>
                ...
                <div class="mb-8" v-if="orderData.payment === 'invoice'">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-6 text-center">
                        ...
                        <p class="text-gray-600 mb-4">Счет №{{ orderData.invoiceNumber }} от {{ formatDate(orderData.invoiceDate) }} отправлен на ваш email</p>
                        <div class="flex flex-col sm:flex-row justify-center gap-3">
                            <a :href="orderData.invoicePdfUrl" target="_blank" class="bg-blue-600 text-white py-2 px-6 rounded-md text-sm font-medium hover:bg-blue-700">
                                <ArrowDownTrayIcon class="size-4 mr-2" />
                            </a>
                            <button class="bg-gray-100 text-gray-800 py-2 px-6 rounded-md text-sm font-medium hover:bg-gray-200">
                                <EnvelopeIcon class="size-4 mr-2" />
                                Отправить повторно
                            </button>
                        </div>
                    </div>
                ...
                <div class="flex flex-col sm:flex-row justify-center gap-4">
                    <NuxtLink to="/catalog" class="bg-blue-600 text-white py-2 px-6 rounded-md text-sm font-medium hover:bg-blue-700 transition-colors">
                        <ArrowLeftIcon class="size-4 mr-2" />
                        Вернуться в каталог
                    </NuxtLink>
                    <NuxtLink to="/account/orders" class="bg-white border border-blue-600 text-blue-600 py-2 px-6 rounded-md text-sm font-medium hover:bg-blue-50">
                        <ListBulletIcon class="size-4 mr-2" />
                        Мои заказы
                    </NuxtLink>
                </div>
            </div>
...
import { ArrowDownTrayIcon, ArrowLeftIcon, ArrowPathIcon, ArrowRightIcon, CheckIcon, DocumentTextIcon, EnvelopeIcon, ListBulletIcon } from "@heroicons/vue/16/solid";
