<template>
  ...
  >
    <!-- Слой-ссылка -->
    <RouterLink :to="`/product/${product.id}`" class="absolute inset-0 z-0">
    </RouterLink>
    <!-- Содержимое карточки -->
    <div class="relative z-10 flex flex-col h-full">
      <!-- Изображение -->
      <div
        class="relative mb-4 pt-[70%] bg-gray-100 rounded-lg overflow-hidden"
      >
        <img
        ...
        <div
          v-if="product.badge"
          class="absolute top-3 left-3 bg-yellow-400 text-yellow-800 text-xs font-bold px-2 py-1 rounded"
        >
          {{ product.badge }}
        </div>
      </div>
...
const getImageUrl = (filename) => {
  if (!filename) return ''
  return `http://localhost:1488/api/files/${filename}`
  // return `${import.meta.env.VITE_API_URL}/files/${filename}`;
};
