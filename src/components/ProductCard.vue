...
<template>
  <div class="group bg-white rounded-xl shadow-sm p-5 transition-all duration-300 hover:shadow-md hover:border-blue-600 hover:border hover:-translate-y-1 border border-transparent h-full flex flex-col">
    <RouterLink 
      :to="`/catalog/${product.id}`"
      class="relative mb-4 pt-[70%] bg-gray-100 rounded-lg overflow-hidden block"
    >
      <img 
        :src="product.image" 
        :alt="product.name" 
    ...
    <div class="flex-grow">
      <RouterLink 
        :to="`/catalog/${product.id}`" 
        class="text-lg font-bold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-2 block mb-2"
      >
        {{ product.name }}
      </RouterLink>
    ...
    <div class="mt-auto">
      <div class="flex justify-between items-center mb-4">
        ...
        <div v-if="product.inStock" class="text-green-600 text-sm font-medium">
          <i class="fas fa-check-circle mr-1"></i> В наличии
        </div>
        <div v-else class="text-gray-500 text-sm">
          <i class="fas fa-clock mr-1"></i> Под заказ
        </div>
      </div>
